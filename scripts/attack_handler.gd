...
func play_attack_animation(delta):
	...
	var flipped = -1 if entity.get_node("AnimationHandler/Sprite2D").flip_h else 1
	...
	if attack_time_elapsed < peak_animation_ratio * total_attack_duration:
		...
		entity.get_node("AnimationHandler/Sprite2D").rotation_degrees = lerp(0, degree_of_attack * flipped, attack_time_normalized)
	else:
		...
		entity.get_node("AnimationHandler/Sprite2D").rotation_degrees = lerp(degree_of_attack * flipped, 0, attack_time_normalized)
